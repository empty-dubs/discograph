...
An interactive graph visualization tool for exploring artists, labels, and releases on [Discogs](https://www.discogs.com).
...
The development build runs a [SvelteKit](https://kit.svelte.dev/) server with a same-origin Discogs API proxy. Credentials live in `.env` on the server and are never exposed to the browser.
...
The contents of the `build/` directory are deployed directly to GitHub Pages.
